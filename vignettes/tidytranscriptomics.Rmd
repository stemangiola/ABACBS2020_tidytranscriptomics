...
```{r}
pbmc_small_tidy <- ABACBS2020tidytranscriptomics::pbmc_small %>% tidy()
```
...
```{r preprocess seurat, warning=FALSE}
pbmc_small_scaled <-
  pbmc_small_tidy_clean %>%
...
pbmc_small_cell_type <-
  pbmc_small_cluster %>%
  left_join(
  	ABACBS2020tidytranscriptomics::cell_type_df, 
  	by = "seurat_clusters"
  )
...
SingleCellExperiment is a very popular container for single cell RNA sequencing data [@sce]. tidySingleCellExperiemnt provides a bridge between the SingleCellExperiment single-cell package [@sce] and the tidyverse [@wickham2019welcome]. It enables the display of the
...
library(igraph)
library(SingleR)
library(tidySingleCellExperiemnt)
```


## Create `tidySingleCellExperiemnt`
...
If a tidyverse-compatible package is not included in the tidySingleCellExperiemnt collection, we can use `as_tibble` to permanently convert `tidySingleCellExperiemnt` into a tibble.
...
A powerful tool we can use with tidySingleCellExperiemnt is `nest`. We can easily perform independent analyses on subsets of the dataset. First we classify cell types in lymphoid and myeloid; then, nest based on the new classification
...
- `tidySingleCellExperiemnt` is an invisible layer that operates on a `SingleCellExperiment` object and enables us to visualise and manipulate data as if it were a tidy data frame.
- `tidySingleCellExperiemnt` object is a `SingleCellExperiment object` so it can be used with any `SingleCellExperiment` compatible method
