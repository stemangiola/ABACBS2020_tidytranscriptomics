...
bibliography: "`r file.path(system.file(package='ABACBS2020tidytranscriptomics', 'vignettes'), 'tidytranscriptomics.bib')`"
vignette: >
...
Background information and setup instructions for this workshop can be found [here](https://stemangiola.github.io/ABACBS2020_tidytranscriptomics/articles/background.html).
...
We can visually compare the significance for all methods. 
...
de_gate <- 
  ...
  tidygate::gate(
    feature,
    edgerQLT_PValue, 
    deseq2_pvalue, gate_list = ABACBS2020tidytranscriptomics::de_gate_gates
  )
...
This enables us to see, for example, that DESeq2 produces a more conservative logFC statistic for the gene `ENSG00000104725`.
...
## Supplementary
Some things we don't have time to cover in Part 1 of this workshop can be found in the [Supplementary material](https://stemangiola.github.io/ABACBS2020_tidytranscriptomics/articles/supplementary.html).


# Part 2 Single-cell RNA-seq

[Background information](https://stemangiola.github.io/ABACBS2020_tidytranscriptomics/articles/background.html#differences-between-bulk-and-single-cell-rna-sequencing-1)
...
The single-cell RNA sequencing data used here is 3000 cells in total, subsetted from 20 samples from 10 peripheral blood mononuclear cell (pbmc) datasets. The datasets include GSE115189 [@Freytag2018], SRR11038995 [@Cai2020, SCP345 (singlecell.broadinstitute.org), SCP424 [@Ding2020], SCP591 [@Karagiannis2020] and 10x-derived 6K and 8K datasets (support.10xgenomics.com/). 
...
We can interact with our object as we do with any tibble. In this case we want to polish an annotation column. We will extract the sample and dataset names from the file name column into separate columns.
...
A key quality control step performed in single cell analysis is assessment of the proportion of mitochondrial transcripts. A high mitochondrial count can indicate the cell is dead or dying and is thus poor quality.
...
We can use tidyverse to easily reshape the data and create beeswarm plots to visualise the mitochondrial content.
...
# select features
integration_features <- SelectIntegrationFeatures(pbmc_scaled_nested$data, nfeatures = 3000) 
...
And we can plot the output as a 3D plot using plotly.
...
And we can plot the clusters as a 3D plot using plotly.
...
We can identify cluster markers (genes) using Seurat.
...
How many cells are classified differently by SingleR when analyses are done by custer (as above) or by cell (omitting the argument `clusters`). Tip: you can answer this question without creating any variable, using left_join.
```
...
```{r}
	...
	tidybulk::reduce_dimensions(sample, transcript, abundance_RNA_scaled, method = "PCA", action="get", top = Inf) %>%
...
bulk_test = 
  ...
  mutate(test = map(
    ...
    ~ .x %>%
      ...
      # Scaling
      tidybulk::identify_abundant(factor_of_interest = groups) %>%
	    tidybulk::scale_abundance() %>%
      
      # Hypothesis test using default method (edgeR)
      tidybulk::test_differential_abundance(~ groups, action="only") %>%
      slice_head(n=5)
...
SingleCellExperiment object as a tidyverse tibble, and provides SingleCellExperiment-compatible *dplyr*, *tidyr*, *ggplot* and *plotly* functions. Below we show how to perform similar steps to what we already did with Seurat, but this time using tidySingleCellExperiment.
...
# Contributing
If you want to suggest improvements for this workshop or ask questions, you can do so as described [here](https://github.com/stemangiola/ABACBS2020_tidytranscriptomics/blob/master/CONTRIBUTING.md).
