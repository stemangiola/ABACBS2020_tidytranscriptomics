...
```{r}
pbmc_tidy <- ABACBS2020tidytranscriptomics::pbmc %>% tidy()
```
...
```{r preprocess seurat, message=FALSE}
...
It is sometime useful to aggregate cell-wise transcript abundance into sudo-bulk samples. It is possible to explore data and perform hypothesis testing with tools and data-source that we are more familiar with.
...
SingleCellExperiment is a very popular container for single cell RNA sequencing data [@sce]. tidySingleCellExperiment provides a bridge between the SingleCellExperiment single-cell package [@sce] and the tidyverse [@wickham2019welcome]. It enables the display of the
...
```{r}
cell_type_df <- ABACBS2020tidytranscriptomics::cell_type_df_sce
cell_type_df
```
...
A powerful tool we can use with tidySingleCellExperiment is `nest`. We can easily perform independent analyses on subsets of the dataset. First we classify cell types in lymphoid and myeloid; then, nest based on the new classification
