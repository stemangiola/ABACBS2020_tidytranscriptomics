...
A key quality control step performed in single cell analysis is assessment of the proportion of mitochondrial transcripts. A high mitochondrial count can indicate the cell is dead or dying and is thus poor quality. A powerful tool we can use with tidyseurat is tidyverse `nest`. We can easily perform independent analyses on subsets of the data, for example, we can nest by dataset to analyse the mitochondrial content.
