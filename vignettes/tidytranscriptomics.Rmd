...
We can select some of the transcripts that look different between methods in the plot using the [*tidygate*](https://github.com/stemangiola/tidygate) package. We can then investigate these transcripts, for example, by visualising the their counts.
...
We can use tidyverse to easily reshape the data and create beeswarm plots to visualise the mitochondrial content. Detected is number of genes, sum is total counts.
...
We load the `aggregate_cells` function below from [GitHub Gist](https://gist.github.com/stemangiola/ebce5a3f931b298611b56680d39c073c) and create a group for each sample.
```{r}
...
tidybulk::reduce_dimensions(sample, transcript, abundance_RNA_scaled, method = "PCA", on="get") %>%
...
We aggregate by sample name and cell type to perform differential expression analysis. 
...
We can treat `pbmc_small_polished` as a [*SingleCellExperiment* object](https://www.bioconductor.org/packages/release/bioc/vignettes/SingleCellExperiment/inst/doc/intro.html) and calculate the log counts.
